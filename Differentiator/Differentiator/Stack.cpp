...
stack_code stack_verifier(Stack** that_stack)
{
    ...
    {
        if (flag_eq && (!isnan((*that_stack)->stack->buffer[(*that_stack)->stack->length]) ||
            !isnan((*that_stack)->stack_copy->buffer[(*that_stack)->stack_copy->length])))
        {
            return STACK_DELETED;
        }
        ...
        else if ((!flag_eq && !isnan((*that_stack)->stack_copy->buffer[(*that_stack)->stack_copy->length])) ||
            (!flag_hash_stack && flag_hash_copy && !flag_hash_stack_buf && flag_hash_copy_buf))
        {
            //printf("Hello\n");
        ...
        else if (!(flag_hash_stack && flag_hash_copy && flag_hash_stack_buf) && flag_hash_copy_buf)
        {
            ...
            while (!isnan((*that_stack)->stack_copy->buffer[i]))
            {
                new_len++;
                i++;
            }
